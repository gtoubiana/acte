...
describe('Constructeur acte.Jour()', function () {
  ...
  it('retourne this.variables.republicain.ac : l\'année républicaine ' + 'en chiffres (ou Undefined)', function () {
    ...
    // Borne mini
    expect(new acte.Jour('22 septembre 1792').variables.republicain.ac).toEqual(1);
    expect(new acte.Jour('1 Vendémiaire an I').variables.republicain.ac).toEqual(1);

    // Borne maxi
    expect(new acte.Jour('31 décembre 1805').variables.republicain.ac).toEqual(14);
    expect(new acte.Jour('10 nivôse an XIV').variables.republicain.ac).toEqual(14);

    // Date négative
    expect(new acte.Jour('-12 frimaire an -XVIII', false).variables.republicain.ac).toEqual(-18);
    expect(new acte.Jour('-12 frimaire an -1685', false).variables.republicain.ac).toEqual(-1685);

    // Retourne la date hors des bornes
    expect(new acte.Jour('frimaire an XVIII', false).variables.republicain.ac).toEqual(18);

    // Min commune de Paris
    expect(new acte.Jour('27 ventose an LXXIX').variables.republicain.ac).toEqual(79);

    // Max commune de Paris
    expect(new acte.Jour('8 prairial an LXXIX').variables.republicain.ac).toEqual(79);

    // Pendant la commune de Paris
    expect(new acte.Jour('25 mai 1871').variables.republicain.ac).toEqual(79);

    // Coverage JjVersGregorien()
    expect(new acte.Jour('11 nivôse an I').variables.republicain.ac).toEqual(1);

    // Saisie erronée
    expect(new acte.Jour('31 frimaire an XVIII').variables.republicain.ac).not.toBeDefined();
    expect(new acte.Jour('0 frimaire an XVIII').variables.republicain.ac).not.toBeDefined();
    expect(new acte.Jour('-00 frimaire an XVIII').variables.republicain.ac).not.toBeDefined();

    // Manque l'année
    expect(new acte.Jour('27 frimaire').variables.republicain.ac).not.toBeDefined();

    // Manque l'année et le jour
    expect(new acte.Jour('frimaire').variables.republicain.ac).not.toBeDefined();

    // Sous la borne mini
    expect(new acte.Jour('21 septembre 1792').variables.republicain.ac).not.toBeDefined();
    expect(new acte.Jour('30 fructidor an 0').variables.republicain.ac).not.toBeDefined();

    // Au delà de la borne maxi
    expect(new acte.Jour('1 janvier 1806').variables.republicain.ac).not.toBeDefined();
    expect(new acte.Jour('11 nivôse an 14').variables.republicain.ac).not.toBeDefined();

    // Min commune de Paris
    expect(new acte.Jour('26 ventose an LXXIX').variables.republicain.ac).not.toBeDefined();
